...
model_generators = list(
  ma = function(params) ModelAverager(estimator=BSTSEstimator(freqs=c(1,2,3,4), niter=500, lags=1),
                                      weighter=LinearWeighter(),
                                      process=SEIRProcess(params)),
  ar1 = function(params) AR(estimator=BSTSEstimator(freqs=c(1), niter=500, lags=1),
                            process=SEIRProcess(params)),
  ar2 = function(params) AR(estimator=BSTSEstimator(freqs=c(1), niter=500, lags=2),
                            process=SEIRProcess(params)),
  ar3 = function(params) AR(estimator=BSTSEstimator(freqs=c(1), niter=500, lags=3),
                            process=SEIRProcess(params))
)
